...
export class SongComponent implements OnInit {
  ...
  // v
  playNext() {
    const currentSongId = this.songData.getSong()?.id
    const currentSongIndex = this.songData.getQueue().findIndex(song => song.id === currentSongId)
    const newQueue = this.songData.getQueue()
    newQueue.splice(currentSongIndex + 1, 0, this.song);
    this.songData.setQueue(newQueue)
    this.toast.success('Песня добавлена в очередь');
  }

  addToPlaylist(playlistId: number) {
    this.userService.addSongToPlaylist(playlistId, this.song).subscribe((user) => {
      if (!user) return
      this.userService.setUser(user)
      this.toast.success('Песня добавлена в плейлист')
    })
  }

  share() {
    const url = `${window.location.host}/song/${this.song.id}`
    navigator.clipboard.writeText(url)
      .then(() => {
        this.toast.success('Ссылка скопирована в буфер обмена')
      })
      .catch(err => {
        this.toast.success('Что-то пошло не так')
      });
  }
